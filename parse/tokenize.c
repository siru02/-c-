...
/*   Updated: 2023/10/07 22:32:58 by hgu              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_token	*tokenize(char *str)
{
	...
	while (*(str + ++len))//len은 실제 구분자 길이보다 1 짧다
	{
		...
		if ((type == -1 && check_ch_is_token(str + len + 1) > 1) || type > 1)//현재가 일반문자이고 다음 문자가 구분자이면(quatation 제외) || 현재문자가 토큰 구분자이면
		{
	...
	//type은 이전의 타입으로 지정해줘야하므로 갱신하지 않는다
	if (len != 0)
		make_token(str, --len, type, token_head);
	return (token_head);
}
