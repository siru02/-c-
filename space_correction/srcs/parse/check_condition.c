...
/*   Updated: 2023/11/18 13:34:31 by hgu              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	make_expansion_fail(t_token *token, int start, int cut)
{
	if (token->expansion_fail != NULL)
		free(token->expansion_fail);
	token->expansion_fail = malloc(cut - start + 2);
	null_guard(token->expansion_fail);
	ft_strlcpy(token->expansion_fail, token->value + start, cut - start + 3);
}
...
int	case_find_in_envp(t_token *token, char *envp, int start, int cut)
{
	...
	make_expansion_fail(token, start, cut);
	free(token->value);
