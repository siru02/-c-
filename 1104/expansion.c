...
/*   Updated: 2023/11/04 17:51:43 by hgu              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*del_quote(char *target, int start, int *quote_cnt)
{
	...
	*quote_cnt = 0;
	...
	while (target[++idx] != '\0')
	{
		...
		{
			ch = '\0';
			target = new_cut_quote(open_quote, idx, target); //만약 quote가 존재하는 경우에 새로 만든다
			if (idx < start)
			{
				printf("test2 : %c\n",target[idx]);
				(*quote_cnt) += 1;
...
{ //envp명단에서 일치하는 확장명령어를 발견했으면 //start는 $의 인덱스, cut은 마지막 문자의 인덱스 //확장한 안에서 
	...
	free(token->value);
	token->value = target;
	//cut - start = 자르기전 문자열의 길이 // hi=test, echo say"$hi$USER"# start = 4 cut = 6 len = 4 //saytest$USER //7부터 탐색해야함
	return (start + len - 1 - malloc_size); //확장에 성공했으면 token->value를 수정하고, 다음에 탐색해야할 인덱스 - 1을 반환한다 //위를 수정하면 반환값도 수정해야한다
} //echo "$test$USER"
...
{ //start는 $의 인덱스 : token->value[idx] == '$', cut은 $로부터 이어지는 마지막인덱스 //확장을 하면서 동시에 ""를 제거한다
	...
	while (bundle->envp[++idx] != NULL) //문자열배열의 마지막원소는 NULL이므로 순회
	{
		...
		{//일치하는 경우엔 $hi@@같은 경우이면 현재 토큰이 $hi와 @@로 나뉜다, 만악 $hi가 존재하지 않으면 token을 삭제한다
			...
			return (after_expantion); //확장 성공하는경우 //앞으로 탐색해야하는 idx를 반환한다
		}
	}// 찾지 못했으면 $~~를 삭제한다
