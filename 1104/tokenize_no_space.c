...
/*   Updated: 2023/11/04 17:51:41 by hgu              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	printf("simple_cmd\n\n");
	printf("cmd : %s len : %zu\n",node->cmd_path, ft_strlen(node->cmd_path));
	for (int i = 0 ; i< node->argv_cnt; i++)
		printf("argv %d : %s len : %zu\n",i,node->cmd_argv[i],ft_strlen(node->cmd_argv[i]));
	printf("\n\n");
}
...
{ //따옴표가 끝나지 않으면 NULL을 보낸다?, 닫히지 않은 quote를 처리하지 않아도 된다
	if (s[idx] == '\'') //순회중에 작은 따옴표를 찾으면
	{
		while (s[idx + 1] && s[idx + 1] != '\'') //무지성 인덱스 증가 //str[idx + 1]는 '나 \0을 가리킨상태로멈추게된다
			idx++;
		if (s[idx + 1] == '\0') //따옴표 닫히지 않은경우 에러문구를 출력하고, 프리하고 트리의 루트가 NULL을 반환하도록 설정
			write(2, "error1\n", 7);//fd 2로 쏘기
	}
...
void	check_tokenize_condition(char **str, int *len, int type, t_token **token_head)
{
...
t_token	*tokenize(char *str)
{
	...
	while (*(str + ++len))//len은 실제 구분자 길이보다 1 짧다
	{
		...
		check_tokenize_condition(&str, &len, type, &token_head); //줄여서 만든 함수
	}
	if (type != WHITE_SPACE && len != 0) //공백이 아니고, 길이가 0이 아니면
		make_token(str, --len, type, &token_head);
	return (token_head);
}
...
int main()
{
	...
	while (1)
	{
		...
		head = tokenize(ret);
		tmp = head;
		// if (head == NULL) //트리아님 토큰의 헤드
		// 	printf("successfully freed\n");
		// else
			tmp = syntax_analyze(head); //tokenize단계에서 quote가 닫히지 않으면 NULL로 입력된다
		// if (tmp == NULL)
		// {
		// 	printf("syntax error\n");
		// 	free(ret);
		...
		head = expansion_main(head, bundle);
