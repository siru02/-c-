...
/*   Updated: 2023/11/14 14:29:09 by hgu              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{//토큰의 헤드는 빈문자열을 들고있다
	...
	while (now != NULL)
	{
		...
		{
			if (before->type == PIPE || (before->type >= 7 && before->type <= 10) || (ft_strncmp(before->value, "cat", 3) == 0 && now->expansion_fail == NULL)) //1106 현재 토큰이 빈 문자열인데 이전 토큰이 리다이렉션이면 프리해주고 실행해야한다
			;
			else
			{
				...
				if (now->expansion_fail != NULL) // 1107
					free(now->expansion_fail); //1107
				free(now); //토큰 free
				now = before;
			}
		}
...
//모든 quote를 삭제하지 않고 현재와 쌍이 맞는 quote만 삭제한다
	...
	while ((ch != '\0' && tmp[idx + 1]) || (ch == '\0' && (idx + 1) < start))
	{
		...
		else if (ch != '\0' && tmp[idx] == ch) //닫는 quote를 찾은경우 //수정필요있음
		{
			...
			token->quote_end = idx - 2;//2; //2개를 지우므로 현재인덱스보다 2개작은 지점을 quote기준으로 삼는다 //1110 -2를 -1로
...
{//확장결과를 split 해야하는 경우이다
	int		idx;
	char	**split;
	t_token	*next;//token의 next포인터를 저장할 포인터변수
	t_token	*tmp; //token의 임시변수

	split = ft_split(str, ' '); //공백을 기준으로 split한다
	next = token->next;
	token->next = NULL;
	tmp = token;
	idx = -1; //1107 첫 토큰은 이미 존재하므로 다음것부터 붙여야한다
	while (split[++idx])
	{
		if (idx == 0 && str[0]!= ' ') //이 경우는 원래 토큰에 붙여야한다 //1107 ~ //화이트스페이스처리
			ft_strlcat(new, split[0], ft_strlen(new) + ft_strlen(split[0]) + 2); //확장한 부분을 붙여넣는다
		else //다른 경우엔 잘라서 다른 토큰으로 이어붙인다
		{
			make_token(split[idx], ft_strlen(split[idx]), -1, &token); //토큰을 만들어 뒤에 붙인다
			tmp = tmp->next; //이번루프에서 만든 토큰을 가리키게 된다
		}
		printf("split[%d] : %s\n",idx, split[idx]);
		free(split[idx]);
	}
	free(split);
	tmp->next = next; //다시 연결시켜준다 1107 echo test$a | ls | cat
}

char	*insert_envp(t_token *token, char *tmp, char *str, int start)
{//start는 붙여넣기할 지점의 idx //$USER -> hgu //echo '$USER'$USER'$USER' //str은 확장결과 문자열
	...
	printf("test_quote_end : %d\n",token->quote_end);
	token->quote_end += str_len; //1108
	new_len = ft_strlen(tmp) + str_len + 1;
	new = malloc(new_len);
	ft_strlcpy(new, tmp, start + 1); //$앞부분을 넣어준다
	if (token->flag == 1) //따옴표밖에서 확장했으면 //1107
		split_after_expansion(token, str, new);//확장결과 이후로 split하여 여러개의 토큰으로 이어붙인다 //1107
	else //따옴표 안에서 확장한경우 뒤에 문자열만 붙여준다
		ft_strlcat(new, str, new_len); //확장한 부분을 붙여넣는다
	ft_strlcat(new, tmp + start, new_len); //확장하지 않은 부분중 뒷부분을 붙여준다
	free(tmp);
	token->expansion_idx = start + str_len; //start는 $의 인덱스, len은 문자열길이 '$USER'$USER -> $USERhgu ->ex_idx = 5 + 3 = 8
	printf("token : %s quote_end : %d expansion_idx : %d\n",new, token->quote_end, token->expansion_idx);
	return (new);
}
...
{//envp에서 찾은경우 //확장하는 부분과 확장한하는 부분으로 나눈다 //확장안하는 부분에서 quote들을 모두 제거한다 //합친다
	...
	quote_cnt = 0;
	len = ft_strlen(str);
	tmp = malloc(ft_strlen(token->value)); //넉넉하게 이렇게 만들어둔다 //필요하면 나중에 수정할것 //free완료
...
int	check_dollar(t_token *token, int idx, t_bundle *bundle)
{
	char	*str;
	int		cut;
	//int		tmp; //1106 token->flag값을 임시저장하는 변수

	str = token->value;
	//tmp = token->flag;
	if (token->flag >= 0 && str[idx] == '$')//현재 인덱스가 $이면 확장을 수행한다
	{
		if (str[idx + 1] == '\0') //$하나로 끝나면 바로 종료
			return (idx);
		// if (token->flag == 0 && str[idx + 1] == '\"') //"$"같은 경우 확장과 quote삭제를 통해 없어지게 되는데 "밖으로 나오는 효과를 주기위해 flag를 1로 초기화한다
		// 	tmp = 1;
			//token->flag = 1;
		cut = cut_env_idx(token->value, idx); //환경변수 마지막 문자의 주소를 반환하도록 한다
		idx = new_expand(token, idx, cut, bundle); //after_expansion을 받는다
		//token->flag = tmp; //1106
	}
	return (idx);
}
...
t_token	*expansion_main(t_token *head, t_bundle *bundle)
{
	t_token *tmp;
	int		idx;
	//int		flag; //flag == 1이면 큰따옴표나 따옴표밖에 있는 상태 -> 확장가능, flag == -1이면 작은 따옴표 안인 상태 -> 확장불가능 //1106 token->flag로 대체
	...
	while (tmp != NULL) //토큰을 마지막까지 탐색한다
	{
		...
		tmp->flag = 1; //flag : 1 이면 따옴표밖 0이면 큰 따옴표안, -1이면 작은 따옴표안1
		while (tmp->value[++idx] != '\0') //토큰의 value 문자열을 탐색한다
		{ //quote_end는 따옴표가 끝나고 검사를 시작해야하는 인덱스를 가리킨다
			printf("현재 단어 : %c flag : %d\n", tmp->value[idx], tmp->flag);
			if (idx == tmp->quote_end + 1) //1110
				tmp->flag = 1; //1110
			if (tmp->flag == 1 && tmp->value[idx] == '\'') //작은 따옴표는 $로 확장이 불가능하므로 flag를 따로 준다
				tmp->flag = -1; //작은 따옴표안에 있는 상태
			else if (tmp->flag == 1 && tmp->value[idx] == '\"') //큰 따옴표만나면 tmp->flag 0으로
				tmp->flag = 0; //큰 따옴표 안에 있는 경우
			else if (tmp->flag == 0 && tmp->value[idx] == '\"') //큰따옴표종료
				tmp->flag = 1; //따옴표 밖에 있는 경우
			else if (tmp->flag == -1 && tmp->value[idx] == '\'')//작은 따옴표 종료
				tmp->flag = 1; //따옴표 밖에 있는 경우
			idx = check_dollar(tmp, idx, bundle); //check_dollar함수는 tmp->value[idx] == '$'이면 확장을 수행하고 마지막 지점의 idx반환
